...
-- создание таблицы users_communities
...
from media m, likes l, profiles p
where l.media_id= m.id and (m.user_id=p.user_id and timestampdiff(day, p.birthday, now())< 12*365);
...
from users u
left join messages mg on u.id = mg.from_user_id
where mg.id is null;
